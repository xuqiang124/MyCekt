...
class CEKT(nn.Module):
    ...
    def forward(self, features, questions):
        ...
        self.adj_matrix = torch.zeros(self.num_kcs,self.num_kcs, device=device)  # 邻接矩阵，用于GAT的知识图谱
        ...
        for t in range(seq_len - 1):
            ...
            # 步骤3：考虑KC间的关联信息
            # 使用KC间的相似度来增强更新
            kc_norm = kc_avg_info / (kc_avg_info.norm(dim=1, keepdim=True) + 1e-8)  # 归一化
            kc_similarity_matrix = torch.mm(kc_norm, kc_norm.t())  # (num_kcs, num_kcs)
            # kc_similarity_matrix = F.softmax(kc_similarity_matrix / 0.1, dim=1)  # 温度参数0.1
            # 设定阈值，比如 threshold = 0.5
            threshold = 0.5
            new_adj_matrix = (kc_similarity_matrix > threshold).int()  # 大于阈值的为1，否则为0
            self.adj_matrix = (self.adj_matrix.int() | new_adj_matrix.int()).int()

            # --- 3. 知识增强 (GAT) ---
            # 真实场景中，GAT的输入是与 e_t 相关的KCs的嵌入
            # 这里简化：GAT在所有KC嵌入上操作，得到增强后的全局KC嵌入
            enhanced_all_kc_embeddings = self.gat(kc_avg_info, self.adj_matrix)
