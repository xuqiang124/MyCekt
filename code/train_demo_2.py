...
class CEKT(nn.Module):
    def __init__(self, qt_one_hot_matrix, questions_embeddings,kc_embeddings, d_k, d_e, d_r, d_h, d_k_enhanced, alpha, nheads, dropout):
        ...
        self.exercise_embedding = nn.Parameter(convert_embeddings_to_tensor(questions_embeddings), requires_grad=False) 
        self.kc_embedding = nn.Parameter(convert_embeddings_to_tensor(kc_embeddings)) 
        print(self.kc_embedding.shape, self.exercise_embedding.shape)
    ...
    def forward(self, features, questions, adj):
        ...
        for t in range(seq_len - 1):
            ...
            enhanced_all_kc_embeddings = self.gat(self.kc_embedding, adj)
